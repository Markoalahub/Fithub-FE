...
export default function DevDashboard({
  ...
  const selectedProposal = useMemo(() => {
    ...
    return (
      pipelineProposals.find((proposal) => proposal.id === selectedProposalId) ??
      pipelineProposals[0]
    );
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm min-h-[620px] space-y-6 relative">
        ...
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs text-emerald-800 space-y-2">
          <p className="font-semibold">
            파이프라인을 보면서 우측 하단 플로팅 버튼으로 기획자와 바로 협의할 수 있습니다.
          </p>
        ...
        {isProposalWidgetOpen && (
          ...
          >
            <div className="px-3 py-2.5 border-b border-gray-100 bg-gray-50 flex items-center justify-between gap-2">
              <p className="text-sm font-semibold text-gray-900">기능 제안 협업 패널</p>
              <button
        ...
        <button
          ...
          className="fixed z-40 inline-flex h-14 w-14 items-center justify-center rounded-full bg-emerald-600 text-white shadow-xl hover:bg-emerald-700 active:cursor-grabbing"
          style={{
