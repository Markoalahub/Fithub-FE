...
export default function DevDashboard({
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            섹션 탭을 통해 파이프라인과 피드백 영역을 분리해 집중도를
            높였습니다.
          </p>
        </div>
        <button
      ...
      <div className="rounded-2xl border border-gray-200 bg-gradient-to-r from-indigo-50 via-white to-cyan-50 p-2 grid grid-cols-1 md:grid-cols-2 gap-2">
        <button
        ...
        >
          <MessageSquare className="h-4 w-4" /> 피드백 타임라인 & 플로우차트
        </button>
      </div>
            ...
            <div className="mt-auto pt-5 border-t border-gray-100">
              {proposalStatus === "discussing" ? (
                ...
                <div className="w-full text-center p-3 bg-green-50 text-green-700 rounded-xl border border-green-200 text-sm font-medium flex items-center justify-center gap-2">
                  <CheckCircle2 className="w-5 h-5" /> 기획자 승인 완료
                  (파이프라인 반영됨)
                </div>
              )}
            </div>
