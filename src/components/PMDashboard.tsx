...
import { Github, Activity, CheckCircle2, AlertCircle, Send, Bot, User, ArrowRight, MessageSquare, ArrowRightLeft, GitPullRequest, ArrowDown, Check } from 'lucide-react';
...
export default function PMDashboard({ features, setFeatures, timelineEvents, setTimelineEvents, proposalStatus, setProposalStatus }: PMDashboardProps) {
  const [chatInput, setChatInput] = useState('');
  const [chatHistory, setChatHistory] = useState([
    { role: 'ai', content: '안녕하세요! 프로젝트 진행 상황이나 기술적인 부분에 대해 궁금한 점을 물어보세요.' }
  ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setChatHistory(prev => [...prev, { 
        role: 'ai', 
        content: 'GitHub 커밋 기록을 확인해본 결과, 현재 결제 모듈 연동 작업이 80% 정도 진행되었습니다. 내일 중으로 테스트 서버에 반영될 예정입니다.' 
      }]);
    }, 1000);
  };
  ...
  const handleFinalConfirm = () => {
    ...
    setFeatures(prev => prev.map(f => {
      if (f.id === 2) { // 결제 모듈 연동
        return {
          ...f,
          tasks: [...f.tasks, { id: '2-4', title: '결제 실패/오류 예외 처리', completed: false, isAiSuggested: true }]
        };
      }
      return f;
    }));
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <header className="mb-6">
        ...
        <p className="text-gray-500 mt-1">프로젝트 현황을 파악하고 개발팀과 소통하세요.</p>
      </header>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col h-[350px]">
        <div className="p-4 border-b border-gray-100 bg-indigo-50/50 rounded-t-2xl flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-indigo-600/70 mt-1">GitHub 데이터와 회의록을 기반으로 개발 현황을 답변해 드립니다.</p>
          </div>
        </div>
          ...
          {features.map((feat, idx) => {
            ...
            return (
              <React.Fragment key={feat.id}>
                <div className={`flex-shrink-0 w-72 p-4 rounded-xl border transition-all flex flex-col h-64 ${
                  status === 'completed' 
                  ...
                  <h4 className={`font-medium text-sm mb-3 truncate ${status === 'completed' ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                    {feat.name}
                  </h4>
                ...
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  <div className="bg-white p-3 rounded-lg border border-gray-200">
                    <div className="flex items-center gap-2 text-blue-500 mb-1">
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-wider">AI 기획적 해석</span>
                    </div>
