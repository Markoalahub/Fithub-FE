...
import { AuthUser, demoAccounts, LoginProvider } from '../types/auth';
...
const toAuthUser = (account: (typeof demoAccounts)[number], provider: LoginProvider): AuthUser => ({
  id: account.id,
  role: account.role,
  name: account.name,
  email: account.email,
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-sky-50 px-6 py-8 text-gray-900">
      <div className="mx-auto flex w-full max-w-md items-center justify-center py-12">
        <section className="w-full rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">{appConfig.name}</h1>
          <h2 className="mt-7 text-xl font-bold text-gray-900">로그인</h2>
          <p className="mt-2 text-sm text-gray-500">소셜 로그인 방식을 선택하고 계정 정보를 입력하세요.</p>
          ...
          <p className="mt-6 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 text-xs text-gray-600">
            테스트용 계정 정보와 역할별 계획은 <code>DEMO_LOGIN_PLAN.md</code>를 참고하세요.
          </p>
        </section>
      </div>
    </div>
